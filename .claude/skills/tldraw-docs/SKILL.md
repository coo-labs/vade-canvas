...
This skill is used heavily inside the `vade-canvas` repo. A few things worth knowing when doing tldraw work there:

- **Current version: tldraw `^4.5.10`** (per `package.json`). The v3 → v4 upgrade landed in #102 (closed #32) and the v4 Hobby License is wired via `VITE_TLDRAW_LICENSE_KEY` for `*.vade-app.dev` builds (#100). Anchor doc lookups to `tldraw.dev/releases/v4.x` and the latest `reference/editor/Editor` page; both reflect what we ship.
...
- **Persistence is IndexedDB by default, R2+D1 via the cloud library.** See `sdk-features/persistence` and `sdk-features/store` for how tldraw snapshots work; the vade-canvas `worker/library.ts` and `mcp/stores/` modules wrap that. Asset bytes do *not* travel in the snapshot — see the asset-persistence note above.
